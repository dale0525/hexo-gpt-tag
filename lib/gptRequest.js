...
const gptRequest = async function (apiKey, text, max_tags, model) {
    // Check cache
    ...
    try {
        const response = await openai.createChatCompletion({
            model: model,
            messages: [
                { 'role': 'system', 'content': `假如你是一名IT网站编辑，擅长总结归纳文章内容。现在你有一项任务，归纳发送给你的文章内容，根据文章的主要话题生成标签。要求：1.单个标签不能中英文混用，如果使用中文，单个标签不得超过4个中文字符；如果使用英文，单个标签不得超过3个英文单词。2.使用更普适的词汇，防止生成含义类似但用语不同的标签。3.生成的标签数量不得超过${max_tags}个。4.不同标签用英文逗号隔开` },
