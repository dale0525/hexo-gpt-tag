...
const process_post = async function (apiKey, max_tags, override, model, data) {
    ...
    let tags_str = await gptRequest(apiKey, text, max_tags, model);
    if (!tags_str)
        return;
...
const process = async function (data) {
    ...
    await process_post(apiKey, max_tags, override, model, data);
