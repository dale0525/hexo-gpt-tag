...
const process_post = async function (
    ...
    let tags = await gptRequest(apiKey, text, max_tags, max_tokens, model, base_url);
    if (!tags) return;
...
const process = async function (data) {
    ...
    const base_url = this.config.gpt_tag.base_url || "https://api.openai.com/v1";

    await process_post(apiKey, max_tags, max_tokens, override, model, data, base_url);
